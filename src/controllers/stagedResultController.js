...
/**
 * Core Engine: Recalculate Scores and Ranks for a Test
 * Can be called internally (e.g., after challenge acceptance) or via HTTP endpoint.
 * Supports:
 * - Single correct answers ('A', 'B', 'C', 'D')
 * - Multiple valid answers ('A,B', 'A or B', 'A|C')
 * - BONUS / DROPPED questions (+marks to all attempted students, 0 negative)
 */
export async function recalculateTestScoresAndRanks(testId) {
  ...
  for (const att of attempts) {
    ...
    for (const [qId, q] of questionMap.entries()) {
      ...
      const isBonus = correctRaw === 'BONUS' || correctRaw === 'DROPPED' || correctRaw === 'ALL';

      if (isBonus) {
        // Official NTA / IISER rule: full positive marks to all candidates who appeared
        totalRawScore += pos;
...
export const calculateTestRanks = async (req, res) => {
  try {
    ...
    const result = await recalculateTestScoresAndRanks(testId);

    return res.status(200).json({
      success: true,
      message: `Calculated ranks for ${result.count} students.`,
      count: result.count,
      liveDenominator: result.liveDenominator
    });
  } catch (err) {
